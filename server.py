...
def regenerate_values(energy, max_energy, last_ts, energy_level=1):
    ...
    new_last = now if new_energy >= max_energy else last + gained * interval
...
def ensure_user(user, referral_code=None):
    ...
    with db() as con:
        ...
        if raw.isdigit():
            candidate = int(raw)
            if candidate != tg_id and con.execute("SELECT telegram_id FROM players WHERE telegram_id=%s", (candidate,)).fetchone():
                ref_id = candidate
        con.execute("""
            INSERT INTO players
        ...
        if ref_id:
            con.execute("""
                UPDATE players SET referrals=referrals+1, coins=coins+1000,
                max_coins=GREATEST(max_coins, coins+1000) WHERE telegram_id=%s
            """, (ref_id,))
...
def api_init(req: InitRequest):
    user=check_init_data(req.init_data); ensure_user(user,req.start_param); return get_state(int(user["id"]))
@app.post("/api/state")
def api_state(req: StateRequest):
    user=check_init_data(req.init_data); return get_state(int(user["id"]))
@app.post("/api/tap")
...
def api_upgrade(req: UpgradeRequest):
    user=check_init_data(req.init_data); tg_id=int(user["id"]); ensure_user(user)
    with db() as con:
        row=con.execute("SELECT coins,max_coins,tap_power,energy_level,max_energy FROM players WHERE telegram_id=%s FOR UPDATE",(tg_id,)).fetchone()
        if req.kind=="tap":
...
def leaderboard():
    with db() as con:
        rows=con.execute("SELECT first_name,username,max_coins FROM players ORDER BY max_coins DESC,telegram_id ASC LIMIT 20").fetchall()
