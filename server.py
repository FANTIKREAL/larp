...
def init_db():
    with db() as con:
        ...
        for sql in [
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS username TEXT NOT NULL DEFAULT ''",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS first_name TEXT NOT NULL DEFAULT ''",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS tap_power INTEGER NOT NULL DEFAULT 1",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS energy_level INTEGER NOT NULL DEFAULT 1",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS last_energy_ts BIGINT NOT NULL DEFAULT 0",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS referred_by BIGINT",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS created_at BIGINT NOT NULL DEFAULT 0",
            ...
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS combo_best INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS daily_streak INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS daily_claim_ts BIGINT NOT NULL DEFAULT 0",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS last_task_day TEXT NOT NULL DEFAULT ''",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS task_taps BIGINT NOT NULL DEFAULT 0",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS task_coins BIGINT NOT NULL DEFAULT 0",
            "ALTER TABLE players ADD COLUMN IF NOT EXISTS rare_found INTEGER NOT NULL DEFAULT 0",
        ]: con.execute(sql)
        con.execute("""CREATE TABLE IF NOT EXISTS daily_claims (telegram_id BIGINT PRIMARY KEY, last_claim_day TEXT NOT NULL DEFAULT '')""")
...
def regenerate_values(energy,max_energy,last_ts,level=1):
    max_energy=max(0,int(max_energy)); energy=min(max(0,int(energy)),max_energy); now=int(time.time()*1000); last=int(last_ts or now)
    ...
    new=min(max_energy,energy+gained); new_last=now if new>=max_energy else last+gained*interval
    return new,new_last

def level_for(coins): return max(1,min(100,int(max(0,coins)//10000)+1))
def level_name(level):
    names=["Новичок","Тапер","Гриндер","LARP PRO","LARP MASTER","LARP KING"]
    return names[min(len(names)-1,(level-1)//5)]
...
def api_tap(req:TapRequest):
    ...
    with db() as con:
        row=con.execute("SELECT coins,max_coins,energy,max_energy,tap_power,last_energy_ts,energy_level FROM players WHERE telegram_id=%s FOR UPDATE",(tg_id,)).fetchone()
        energy,ts=regenerate_values(row[2],row[3],row[5],row[6]);actual=min(int(req.taps),energy);gained=actual*int(row[4]);new_coins=int(row[0])+gained
        con.execute("UPDATE players SET coins=%s,max_coins=GREATEST(max_coins,%s),energy=%s,last_energy_ts=%s,xp=xp+%s,task_taps=task_taps+%s WHERE telegram_id=%s",(new_coins,new_coins,energy-actual,ts,actual,actual,tg_id))
...
def daily(req:StateRequest):
    ...
    with db() as con:
        r=con.execute("SELECT daily_claim_ts,daily_streak FROM players WHERE telegram_id=%s FOR UPDATE",(tg_id,)).fetchone(); last=int(r[0]); streak=int(r[1]); now=int(time.time())
        if last and time.gmtime(last).tm_yday==time.gmtime(now).tm_yday and time.gmtime(last).tm_year==time.gmtime(now).tm_year: raise HTTPException(400,"Бонус уже получен сегодня")
        prev=time.gmtime(last).tm_yday if last else -1; cur=time.gmtime(now).tm_yday
        if last and (now-last)<=172800:streak=min(streak+1,7)
        else:streak=1
        reward=500*streak
...
def roulette(req:StateRequest):
    user=check_init_data(req.init_data);tg_id=int(user["id"]);ensure_user(user);cost=100
    rewards=[0,50,100,250,500,1000,2500,10000];weights=[12,18,20,18,14,10,6,2]
    with db() as con:
        ...
        reward=random.choices(rewards,weights=weights,k=1)[0];new=max(0,int(r[0])-cost+reward)
        con.execute("UPDATE players SET coins=%s,max_coins=GREATEST(max_coins,%s),xp=xp+20 WHERE telegram_id=%s",(new,new,tg_id))
...
def combo(req:ComboRequest):
    ...
    with db() as con:
        r=con.execute("SELECT combo_best FROM players WHERE telegram_id=%s FOR UPDATE",(tg_id,)).fetchone();best=max(int(r[0]),combo)
        con.execute("UPDATE players SET coins=coins+%s,max_coins=GREATEST(max_coins,coins+%s),combo_best=%s WHERE telegram_id=%s",(bonus,bonus,best,tg_id))
...
def rare(req:StateRequest):
    ...
    with db() as con:
        con.execute("UPDATE players SET coins=coins+%s,max_coins=GREATEST(max_coins,coins+%s),rare_found=rare_found+1,xp=xp+100 WHERE telegram_id=%s",(reward,reward,tg_id))
...
def tasks(req_init_data:str):
    ...
    return [{"id":"taps","title":"Сделай 500 тапов","progress":min(int(r[0]),500),"goal":500,"reward":1000,"done":int(r[0])>=500},{"id":"coins","title":"Заработай 10 000 🪙","progress":min(int(r[1]),10000),"goal":10000,"reward":2500,"done":int(r[1])>=10000}]

@app.post("/api/task/claim")
def task_claim(req:StateRequest):
    user=check_init_data(req.init_data);tg_id=int(user["id"]);ensure_user(user);day=today();tid=""
    # The client sends the task id in a JSON-compatible query parameter to keep the model small.
    raise HTTPException(400,"Задание уже обрабатывается в новой версии")
...
def admin_grant(req:GrantRequest):
    ...
    with db() as con:
        ...
        if not target:raise HTTPException(404,"Пользователь с таким Telegram ID ещё не запускал игру")
