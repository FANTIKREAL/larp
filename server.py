...
def energy_interval_ms(energy_level: int) -> int:
    return max(100, 500 - (max(1, int(energy_level)) - 1) * 50)
...
def regenerate_values(energy, max_energy, last_ts, energy_level=1):
    ...
    interval=energy_interval_ms(energy_level); elapsed=max(0,now-last); gained=elapsed//interval
    if gained<=0:return energy,last
    new_energy=min(max_energy,energy+gained); new_last=now if new_energy>=max_energy else last+gained*interval
...
def ensure_user(user, referral_code=None):
    ...
    with db() as con:
        ...
        con.execute("""INSERT INTO players (telegram_id,username,first_name,coins,max_coins,energy,max_energy,referrals,tap_power,energy_level,last_energy_ts,referred_by,created_at) VALUES (%s,%s,%s,0,0,1000,1000,0,1,1,%s,%s,%s)""",(tg_id,user.get("username",""),user.get("first_name",""),now,ref_id,now))
        if ref_id: con.execute("UPDATE players SET referrals=referrals+1, coins=coins+1000, max_coins=GREATEST(max_coins,coins+1000) WHERE telegram_id=%s",(ref_id,))

def get_state(tg_id):
    with db() as con:
        r=con.execute("SELECT telegram_id,username,first_name,coins,max_coins,energy,max_energy,tap_power,energy_level,referrals,last_energy_ts FROM players WHERE telegram_id=%s",(tg_id,)).fetchone()
        if not r: raise HTTPException(404,"Пользователь не найден")
        energy,new_ts=regenerate_values(r[5],r[6],r[10],r[8]); con.execute("UPDATE players SET energy=%s,last_energy_ts=%s WHERE telegram_id=%s",(energy,new_ts,tg_id))
        return {"id":r[0],"name":r[2] or r[1] or "Игрок","coins":int(r[3]),"best_coins":int(r[4]),"energy":int(energy),"max_energy":int(r[6]),"tap_power":int(r[7]),"energy_level":int(r[8]),"referrals":int(r[9]),"is_admin":tg_id in ADMIN_IDS}
...
def api_tap(req:TapRequest):
    ...
    with db() as con:
        ...
        energy,ts=regenerate_values(row[2],row[3],row[5],row[6]); actual=min(int(req.taps),energy); gained=actual*int(row[4]); new_coins=int(row[0])+gained; new_best=max(int(row[1]),new_coins)
        con.execute("UPDATE players SET coins=%s,max_coins=%s,energy=%s,last_energy_ts=%s WHERE telegram_id=%s",(new_coins,new_best,energy-actual,ts,tg_id))
...
def leaderboard():
    with db() as con: rows=con.execute("SELECT first_name,username,max_coins FROM players ORDER BY max_coins DESC,telegram_id ASC LIMIT 20").fetchall()
...
def admin_grant(req:GrantRequest):
    ...
    with db() as con:
        ...
        new_coins=int(target[0])+int(req.amount); new_best=max(int(target[1]),new_coins)
...
def admin_withdraw(req:GrantRequest):
    ...
    with db() as con:
        ...
        if not target: raise HTTPException(404,"Пользователь с таким Telegram ID ещё не запускал игру")
        new_coins=max(0,int(target[0])-int(req.amount))
        con.execute("UPDATE players SET coins=%s WHERE telegram_id=%s",(new_coins,req.target_id))
    return get_state(req.target_id)
